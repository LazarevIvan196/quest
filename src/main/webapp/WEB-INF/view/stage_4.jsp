...
<head>
    ...
    <style>
        body {
            background-image: url('${pageContext.request.contextPath}/images/stage_4.jpg');
            ...
            fetch("${pageContext.request.contextPath}/startGame", {
                ...
                .then(data => {
                    ...
                    if (data.redirectToStage) {
                        window.location.href = "${pageContext.request.contextPath}/stage_7";
                    }
                })
                .catch(error => {
        ...
        function handleActionTwo() {
            fetch("${pageContext.request.contextPath}/startGame", {
                ...
                .then(data => {
                    ...
                    if (data.redirectToStage) {
                        window.location.href = "${pageContext.request.contextPath}/happy_end";
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
...
<div class="container">
    <form action="${pageContext.request.contextPath}/startGame" method="post" style="text-align: center;">
        <h1>Уровень 4 НОВОЕ НАЧАЛО...</h1>
        <p id="stageText">
            <%
                Map<String, Object> stage0Data = JsonService.readJson("/questOne.json");
                if (stage0Data != null) {
                    Object stage0Obj = stage0Data.get("STAGE_4");
        ...
        <div class="button" style="display: flex; justify-content: space-between; margin-top: 10px;">
            <button type="button" onclick="handleActionOne()">Отправить детей в шахты</button>
            <button type="button" onclick="handleActionTwo()">Объявить круглосуточные смены</button>
